...
function StageRow({ stage }: {
  stage: { order: number; title: string; titleAr: string; hours: number; status: string; progress: number }
}) {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-colors
      ${isActive ? 'border-cyan-400/20 bg-cyan-400/3' : 'border-white/5 bg-white/[0.02] hover:border-cyan-400/10'}
      ${isUpcoming ? 'opacity-50' : ''}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${dotClass}`}>
        {isDone ? '✓' : stage.order}
      </div>
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium truncate ${isActive ? 'text-cyan-400' : 'text-white/80'}`}>{stage.title}</p>
      ...
      {isDone ? (
        <span className="text-xs px-2 py-0.5 rounded-full bg-green-400/10 text-green-400 border border-green-400/20 flex-shrink-0">Done ✓</span>
      ) : !isUpcoming ? (
        <div className="w-20 flex-shrink-0">
          <p className="text-xs text-white/30 text-right mb-1">{stage.progress}%</p>
          <div className="h-1 rounded-full bg-white/6 overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400" style={{ width: `${stage.progress}%` }} />
          </div>
        </div>
      ) : (
        <div className="w-20 flex-shrink-0">
...
function ActivityHeatmap({ levels }: { levels: number[] }) {
  const colorMap: Record<number, string> = {
    0: 'bg-white/4', 1: 'bg-cyan-400/15', 2: 'bg-cyan-400/30', 3: 'bg-cyan-400/55', 4: 'bg-cyan-400/85',
  }
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {levels.map((lvl, i) => (
          <div key={i} title={lvl === 0 ? 'No activity' : `Level ${lvl}`}
            className={`aspect-square rounded-sm ${colorMap[lvl] ?? 'bg-white/4'}`} />
        ))}
      </div>
    </div>
  )
...
function AchievementBadge({ badge }: {
  badge: { icon: string; name: string; nameAr: string; desc: string; rarity: string }
}) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/[0.025] border border-white/5">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0 border ${iconBg}`}>{badge.icon}</div>
      <div className="flex-1 min-w-0">
...
function OverviewPanel({ userName }: { userName: string }) {
  const { stats, stages, streak, achievements } = useDashboard()
  ...
  // Map real stages to StageRow shape; fallback to embedded mock stages
  const stageRows = stages.length
    ? stages.map((s: any, i: number) => ({
        order: s.stage_order ?? i + 1,
        title: s.title,
        titleAr: s.titleAr ?? s.title_ar ?? '',
        hours: s.estimated_hours ?? 0,
        status:
          s.status === 'completed' ? 'done'
          : s.status === 'in_progress' ? 'active'
          : 'upcoming',
        progress: s.progressPercent ?? s.progress_percentage ?? 0,
      }))
    : [
        { order: 1, title: 'Foundation',         titleAr: 'الأساسيات',       hours: 20, status: 'done',     progress: 100 },
        { order: 2, title: 'Core Skills',         titleAr: 'المهارات الأساسية', hours: 24, status: 'done',     progress: 100 },
        { order: 3, title: 'Advanced Topics',     titleAr: 'المواضيع المتقدمة', hours: 18, status: 'active',   progress: 68  },
        { order: 4, title: 'Portfolio Projects',  titleAr: 'مشاريع المحفظة',  hours: 32, status: 'upcoming', progress: 0   },
        { order: 5, title: 'Job Ready',           titleAr: 'جاهز للتوظيف',    hours: 20, status: 'upcoming', progress: 0   },
      ]

  // Activity heatmap: use streak data if available
  const activityLevels = [0,0,1,2,3,4,2,1,3,4,3,2,1,0,2,3,4,3,2,1,3,4,3,2,1,2,3,1]

  // Achievements: use real if available
  const badgeList = achievements.length
    ? achievements.map((a) => ({
        icon: a.icon ?? '🏆',
        name: a.name,
        nameAr: a.name,
  ...
  const longestStreak = streak?.longest_streak ?? 42
  ...
  return (
    <div className="space-y-4 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/40 text-xs mt-2">
              Track: <span className="text-cyan-400">{stats.selectedTrack}</span>
              <span className="text-white/15 mx-2">·</span>
              <span className="font-arabic text-white/30">{stats.selectedTrackAr}</span>
            </p>
          </div>
          <div className="flex items-center gap-5 flex-shrink-0">
            <div className="text-center">
              <ProgressRing percent={stats.overallProgress} size={80} color="gradient" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon="🔥" value={`${stats.streakDays}d`}        label="Streak"       labelAr="أيام متتالية"    color="text-orange-400" />
        <StatCard icon="⏱" value={`${stats.totalHoursLearned}h`}  label="Hours Learned" labelAr="ساعات تعلم"     color="text-cyan-400"   />
          ...
          <GlassCard padding={false} className="p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-base font-semibold text-white">{stats.selectedTrack} Path</p>
                <p className="text-xs text-white/30 mt-0.5">{stats.totalStages} stages</p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold gradient-text">{stats.overallProgress}%</p>
                <p className="text-xs text-white/30">Complete</p>
              </div>
            </div>
            <div className="h-2 rounded-full bg-white/5 overflow-hidden mb-1">
              <div className="h-full rounded-full progress-gradient transition-all duration-700" style={{ width: `${stats.overallProgress}%` }} />
            </div>
            <div className="flex justify-between text-xs text-white/25 mb-5">
              <span>Stage {stats.completedStages} of {stats.totalStages}</span>
              <span>{stats.nextMilestone}</span>
            </div>
            <div className="space-y-2">
              {stageRows.map((s) => <StageRow key={s.order} stage={s} />)}
            </div>
          </GlassCard>
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-sm font-semibold text-white">
                  ...
                  <span className="font-arabic text-white/35 text-xs">{stats.groupNameAr}</span>
                </p>
              ...
              {mockGroupMembers.slice(0, 6).map((m) => (
                <div key={m.id} className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
                  <div className="relative flex-shrink-0">
                    <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${m.avatarColor} flex items-center justify-center text-white text-xs font-bold`}>{m.initials}</div>
                    {m.isOnline && <span className="absolute bottom-0 right-0 w-2 h-2 bg-green-400 rounded-full border-2 border-[#0B1120]" />}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="flex-shrink-0 w-16">
                    <div className="h-1 rounded-full bg-white/6 overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400" style={{ width: `${m.progressPercent}%` }} />
                    </div>
        ...
        <div className="space-y-4">
          ...
          <GlassCard neonBorder padding={false} className="p-4">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-cyan-400/8 border border-cyan-400/20 flex items-center justify-center text-xl flex-shrink-0">🎯</div>
              <div className="flex-1 min-w-0">
                <p className="text-white/35 text-xs mb-0.5">Next Milestone · الهدف القادم</p>
                <p className="text-white text-sm font-medium truncate">{stats.nextMilestone}</p>
              </div>
              <ProgressRing percent={weeklyPct} size={48} strokeWidth={4} color="cyan" label="wk" className="flex-shrink-0" />
            </div>
          </GlassCard>
          ...
          <GlassCard padding={false} className="p-5">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <p className="text-3xl font-bold text-orange-400 leading-none">{stats.streakDays}</p>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div className="text-center py-3 rounded-xl bg-white/[0.025] border border-white/5">
                <p className="text-lg font-bold text-orange-400">{longestStreak}</p>
                <p className="text-xs text-white/30">Longest streak</p>
              </div>
              <div className="text-center py-3 rounded-xl bg-white/[0.025] border border-white/5">
                <p className="text-lg font-bold text-cyan-400">{stats.totalHoursLearned}</p>
                <p className="text-xs text-white/30">Hours learned</p>
              </div>
            </div>
            <ActivityHeatmap levels={activityLevels} />
          </GlassCard>
          ...
          <GlassCard padding={false} className="p-5">
            ...
            <div className="space-y-2">
              {badgeList.map((b) => <AchievementBadge key={b.name} badge={b} />)}
            </div>
          </GlassCard>
      ...
      <header className="sticky top-0 z-50 glass-card rounded-none border-x-0 border-t-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <ProfileDropdown />
            <button onClick={handleLogout} className="text-white/40 hover:text-white/70 text-xs transition-colors hidden sm:block">
              Logout · خروج
            </button>
            <button className="md:hidden text-white/60 hover:text-white transition-colors ml-1" onClick={() => setMobileMenuOpen((v) => !v)}>
              {mobileMenuOpen ? '✕' : '☰'}
            </button>
          </div>
        </div>
